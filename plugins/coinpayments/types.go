...
type createInvoiceRequest struct {
	...
	} `json:"items"` // Array of items that a buyer intends to purchase from the merchant
	Amount *struct {
		Breakdown *struct {
	...
	} `json:"amount"` // Total invoice amount with a breakdown that provides details such as total item amount, total tax amount, shipping, handling, insurance, and discounts, if any.
	...
	} `json:"emailDelivery"` // Email delivery options of a merchant invoice
	...
	Buyer       *struct {
		...
		} `json:"name"` // No further information available.
		...
		} `json:"address"` // No further information available.
	} `json:"buyer"` // the buyer information
	...
	Shipping    *struct {
		...
		} `json:"name"` // shipping recipient name
	...
	} `json:"shipping"` // Invoice shipping address and method
	...
	} `json:"merchantOptions"` // Options to show/hide merchant information on an invoice, or include additional merchant information specific to an invoice
	...
	} `json:"metadata"` // Represents metadata information related to an invoice, including integration and hostname details.
	...
	} `json:"webhooks"` // Contains an array of
	...
	} `json:"payoutOverrides"` // Optionally specifies payout configs for this invoice
	...
	} `json:"payment"` // Request to create an invoice payment with predefined currency
